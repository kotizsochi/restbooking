...
const STAFF = [
  { name: "Денис", email: "admin@restbooking.ru", role: "Создатель", registered: "14.05.2026" },
];
...
function VenuesTab() {
  const [filter, setFilter] = useState<"all" | "new" | "confirmed" | "seated">("all");
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", minHeight: "calc(100vh - 56px)" }}>
      ...
      <aside style={{ background: "var(--color-bg-card)", padding: 16, boxShadow: "var(--shadow-sm)" }}>
        ...
        <div style={{ fontSize: 11, color: "var(--color-primary)", marginBottom: 16 }}>14 дней до отключения тестового периода</div>
      ...
      <main style={{ padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--color-text-muted)" }}>Ваш виджет для гостей: <span style={{ color: "var(--color-primary)" }}>beluga.restbooking.ru</span></p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-secondary btn-sm" style={{ gap: 4 }}><Settings size={14} /> Общие настройки</button>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          {([["all", "Все"], ["new", "Новые"], ["confirmed", "Подтвержденные"], ["seated", "Пришли"]] as const).map(([key, label]) => (
            <button key={key} className={`time-slot ${filter === key ? "active" : ""}`} style={{ padding: "6px 14px", fontSize: 12 }}
              onClick={() => setFilter(key)}>
              {label} {key === "new" && <span style={{ marginLeft: 4, padding: "1px 5px", borderRadius: 8, background: "var(--color-warning)", color: "#fff", fontSize: 10 }}>{MOCK_BOOKINGS.filter((b) => b.status === "new").length}</span>}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map((b) => {
            ...
            return (
              <div key={b.id} style={{ padding: "14px 16px", background: "var(--color-bg-card)", borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-xs)", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", transition: "box-shadow var(--transition-fast)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {b.comment && <p style={{ fontSize: 12, color: "var(--color-text-muted)", marginTop: 4 }}>{b.comment}</p>}
                </div>
...
function TariffsTab() {
  ...
  const tariffs = [
    ...
    { name: "PRO +", base: 5190, features: ["Все из PRO, плюс:", "Выключение лейбла", "Продажа билетов", "Сертификаты", "Промокоды на скидку", "Интеграции с POS: iiko, r_keeper", "Интеграции с CRM: Битрикс24", "API RESTBooking"] },
  ];
